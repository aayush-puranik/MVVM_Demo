...
class RecipeListAdapter(private val interaction: Interaction? = null) :
    ...
    constructor(
        ...
        fun bind(item: RecipeModel) = with(itemView) {
            itemView.setOnClickListener {
                interaction?.onItemSelected(adapterPosition, item)
            }
            itemView.findViewById<TextView>(R.id.titleTV).text = item.name
            itemView.findViewById<TextView>(R.id.descriptionTV).text = item.description
        }
    }

    interface Interaction {
        fun onItemSelected(position: Int, item: RecipeModel)
    }
}